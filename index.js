...
app.post('/api/login', (req, res) => {
    ...
    db.query("SELECT * FROM user WHERE username = ?",
        ...
        (err, result) => {
            if (err) { console.log(err); }
            if (result.length <= 0) { res.send({message: "Invalid username!", error: true}); }
            else {
        ...
        (err, result) => {
            ...
            for(i = 0; i < result.length; ++i) {
                ...
                for (j = 0; j < parsedUID.length; ++j) {
                    if (parsedUID[j] === userID) { getRoom.push(result[i]); break; }
                } 
            }
            res.send(getRoom); 
        }
...
app.get('/api/join', (req, res) => {
    db.query("SELECT * from room WHERE privacy = ?",
        "public",
        (err, result) => { if (err) {console.log(err);} res.send(result); }
    );
});
...
app.post('/api/post_topic', (req, res) => {
    ...
    db.query("INSERT INTO topic (title, detail, room_id, made_by) VALUES (?,?,?,?)",
        ...
        (err, result) => { if (err) {console.log(err);} res.send("Topic posted successfully!"); }
    );
});
...
app.post('/api/rate/like', (req, res) => {
    ...
    db.query("SELECT * from comment WHERE id = ?",
        ...
        (err, result) => {
            ...
            if (parsedUID.length == 1 && parsedUID[0] == '') { parsedUID[0] = userID; ++likeCount; }
            else {
                for (i = 0; i < parsedUID.length; ++i) {
                    if (parsedUID[i] == userID) { parsedUID.splice(i, 1); --likeCount; break; }
                }
            }
...
app.post('/api/rate/dislike', (req, res) => {
    ...
    db.query("SELECT * from comment WHERE id = ?",
        ...
        (err, result) => {
            ...
            if (parsedUID.length == 1 && parsedUID[0] == '') { parsedUID[0] = userID; ++dislikeCount; }
            else {
                for (i = 0; i < parsedUID.length; ++i) {
                    if (parsedUID[i] == userID) { parsedUID.splice(i, 1); --dislikeCount; break; }
                }
            }
